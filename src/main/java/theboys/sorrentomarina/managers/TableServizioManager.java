...
public class TableServizioManager extends TableManager implements ServizioManager{
  ...
  public void create(int id, boolean bar, boolean ristorante, boolean animazione, boolean wifi, boolean cabina, boolean beach_volley, boolean canoa)throws SQLException{
    ...
    if(bar)bar_=1;else bar_=0;
    if(ristorante)ristorante_=1;else ristorante_=0;
    if(animazione)animazione_=1;else animazione_=0;
    if(wifi)wifi_=1;else wifi_=0;
    if(cabina)cabina_=1;else cabina_=0;
    if(beach_volley)beach_volley_=1;else beach_volley_=0;
    if(canoa)canoa_=1;else canoa_=0;
  ...
  public void update(Servizio servizio) throws SQLException{
    int bar_,ristorante_,animazione_,wifi_,cabina_,beach_volley_,canoa_;
    if(servizio.isBar())bar_=1;else bar_=0;
    if(servizio.isRistorante())ristorante_=1;else ristorante_=0;
    if(servizio.isAnimazione())animazione_=1;else animazione_=0;
    if(servizio.isWifi())wifi_=1;else wifi_=0;
    if(servizio.isCabina())cabina_=1;else cabina_=0;
    if(servizio.isBeach_volley())beach_volley_=1;else beach_volley_=0;
    if(servizio.isCanoa())canoa_=1;else canoa_=0;
